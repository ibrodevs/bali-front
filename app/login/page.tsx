  ...
  return (
    <div style={{ background: bg, color: fg, minHeight: '100vh' }}>
      ...
      <div className="br-auth-shell" style={{ maxWidth: 460, margin: '64px auto', padding: '0 24px' }}>
        <BREyebrow>BALI-RENT · {t.auth.login.toUpperCase()}</BREyebrow>
        <h1 className="br-display" style={{ fontSize: 'clamp(40px, 10vw, 56px)', lineHeight: 0.98, letterSpacing: '-0.03em', margin: '12px 0 32px' }}>{t.auth.login}</h1>
        <form onSubmit={onSubmit} style={{ display: 'grid', gap: 12 }}>
          ...
          <BRPrimary type="submit" disabled={submitting} full style={{ marginTop: 8, padding: '16px' }}>{submitting ? t.common.loading : t.auth.loginCta}</BRPrimary>
        </form>
        <p style={{ fontSize: 14, color: sub, marginTop: 28 }}>
          {t.auth.noAccount} <Link href="/register" style={{ color: fg, textDecoration: 'underline' }}>{t.auth.register}</Link>
        </p>
      </div>
      <SiteFooter />
    </div>
